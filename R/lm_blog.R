...
#'
#' \emph{lm_blog} is used to fit Simple Linear Regression and Multiple Linear Regression models with continous or categorical covariates.
#' @param \emph{eqn} Equation of the model to be fitted. The argument is of the form :Y ~ X1 + X2... + Xn.
#' @param \emph{dataset} Dataframe containing the covariates and response variable as column names.
#' @details \emph{lm_blog} takes an equation and a dataframe as arguments to fit a linear regression model represented by the equation.
#' The dataframe should contain the response variable and the covariates present in the equation as column names.\cr The covariates are treated as continous or categorical. lm_blog automatically converts any non numeric data time into dummy variables using one-got encoding.\cr \emph{lm_blog} then fits the model using the method of least squares
#' @return \emph{lm_blog} returns a list "lr". The list "lr" contains the following components:\cr
#' \tabular{rrrrr}{
...
lm_blog <- function(eqn,dataset){
  ...
  #Removing observations containing NA
  dataset<-na.omit(dataset)

  #Extracting the response variable and covariates from the dataset using the equation
  eqn <- as.character(substitute((eqn)))[2]
  y <- dataset[strsplit(eqn," ~")[[1]][1]]
  x <- dataset[strsplit(eqn," ")[[1]][seq(3,length(strsplit(eqn," ")[[1]]),2)]]

  #Converting non numeric covariates into binary dummy variables
  ...
  #Calculating the coefficient estimates by method of least squares
  Intercept <- rep(1,nrow(x))
  x <- cbind(Intercept,x)
  beta_hat <- solve(as.matrix(t(x))%*%as.matrix(x))%*%as.matrix(t(x))%*%as.matrix(y)

  #Calculating the X-matrix and fitted values
